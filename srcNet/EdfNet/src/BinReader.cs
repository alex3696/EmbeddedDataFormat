...
public class BinReader : BaseReader
{
    ...
    public UInt16 Pos; // Позиция в текущем блоке данных, указывает на количество байт, уже прочитанных из блока
    ...
    public bool ReadBlock()
    {
        ...
        do
        {
            t = (BlockType)_br.ReadByte(); // Читаем байт из потока и пытаемся интерпретировать его как тип блока данных
        }
        while (!Enum.IsDefined(t)); // Если прочитанный байт не соответствует определенному типу блока данных,
                                    // продолжаем читать, пока не получим валидный тип

        var seq = _br.ReadByte(); // Читаем следующий байт, который представляет собой номер блока данных (последовательность)
        var len = _br.ReadUInt16(); // Читаем следующие два байта, которые представляют собой длину данных в блоке (количество байт данных)
        ...
        if (0 < len) // Если длина данных больше нуля, то мы можем прочитать данные из блока
        {
            ...
            {
                ushort fileCrc = _br.ReadUInt16(); // Читаем следующие два байта, которые представляют собой CRC контрольную сумму для данных в блоке
                ushort crc = ModbusCRC.Calc([(byte)_current.Type]); // Вычисляем CRC контрольную сумму для типа блока данных
                crc = ModbusCRC.Calc([_current.Seq], crc); // Обновляем контрольную сумму для номера блока данных, используя предыдущую контрольную сумму
                crc = ModbusCRC.Calc(BitConverter.GetBytes(_current.Qty), crc); // Обновляем контрольную сумму для длины данных, используя предыдущую контрольную сумму
                crc = ModbusCRC.Calc(_current.Data, crc); // Обновляем контрольную сумму для данных в блоке, используя предыдущую контрольную сумму
                if (crc != fileCrc) // Если вычисленная контрольная сумма не совпадает с контрольной суммой, прочитанной из потока,
                    throw new Exception($"Wrong CRC block {_current.Seq}"); // выбрасываем исключение, указывающее на ошибку CRC для текущего блока данных
            }
            ...
            Pos = 0; // Сбрасываем позицию в текущем блоке данных, указывая, что мы еще не прочитали ни одного байта данных из блока
    ...
    public Header? ReadHeader()
    {
        if (_current.Type == BlockType.Header)
            return Header.Parse(_current.Data); // Если тип текущего блока данных является заголовком, то мы пытаемся распарсить данные блока как заголовок и возвращаем его
        return null;
    }
    ...
    public TypeRec? ReadInfo()
    {
        ...
        {
            TypeRec rec = new(); // Создаем новый экземпляр TypeRec для хранения информации о типе данных
            EdfErr err; // Переменная для хранения результата попытки чтения данных из блока данных
    ...
    {
        rest = b; // Инициализируем переменную rest, которая будет использоваться для отслеживания оставшихся
                  // данных после чтения информации о типе данных. Изначально rest указывает на весь входной массив байтов b.
        ...
        rest = rest.Slice(1); // Обновляем переменную rest, чтобы она указывала на оставшуюся часть массива байтов после чтения типа данных.
                              // Теперь rest указывает на массив байтов, начиная со второго элемента.
        // dim
        var dimsCount = rest[0]; // Читаем следующий байт из массива байтов, который представляет собой количество измерений (dimsCount) для типа данных.
        ...
        if (0 < dimsCount) 
        {
            dims = new uint[dimsCount]; // Выделяем массив uint для хранения размеров измерений,
                                        // размер которого соответствует количеству измерений (dimsCount).
            for (int i = 0; i < dimsCount; i++)
            {
                dims[i] = BinaryPrimitives.ReadUInt32LittleEndian(rest); //Читаем 4 байта из массива байтов rest и интерпретируем их как беззнаковое целое число (uint) в формате little-endian, 
                                                                         // которое представляет размер текущего измерения. Сохраняем это значение в массиве dims. 
                rest = rest.Slice(sizeof(UInt32)); // Обновляем переменную rest, чтобы она указывала на оставшуюся часть массива байтов после чтения размера текущего измерения. 
                                                   // Теперь rest указывает на массив байтов, начиная с позиции после прочитанных 4 байт для текущего измерения.
            }
        }
        // name
        byte bNameSize = rest[0]; // Читаем следующий байт из массива байтов rest, который представляет собой размер имени (bNameSize) для типа данных.
        rest = rest.Slice(1); // Обновляем переменную rest, чтобы она указывала на оставшуюся часть массива байтов после чтения размера имени. 
                              // Теперь rest указывает на массив байтов, начиная с позиции после прочитанного байта для размера имени.
        if (255 < bNameSize)
            throw new ArgumentException("name len mismatch");
        var name = Encoding.UTF8.GetString(rest.Slice(0, bNameSize)); // Читаем bNameSize байтов из массива байтов rest, начиная с позиции 0,
                                                                      // и интерпретируем их как строку в кодировке UTF-8, которая представляет имя для типа данных.
                                                                      // Сохраняем это значение в переменной name.
        rest = rest.Slice(bNameSize); // Обновляем переменную rest, чтобы она указывала на оставшуюся часть массива байтов после чтения имени. 
                                      // Теперь rest указывает на массив байтов, начиная с позиции после прочитанных байтов для имени.
        // childs
        List<TypeInf>? childs = null; 
        if (PoType.Struct == type && 0 < rest.Length)
        {
            byte childsCount = rest[0]; // Читаем следующий байт из массива байтов rest, который представляет собой количество дочерних элементов (childsCount)
                                        // для типа данных, если тип данных является структурой (PoType.Struct).
            rest = rest.Slice(1); // Обновляем переменную rest, чтобы она указывала на оставшуюся часть массива байтов после чтения количества дочерних элементов. 
                                  // Теперь rest указывает на массив байтов, начиная с позиции после прочитанного байта для количества дочерних элементов.
            childs = new List<TypeInf>(childsCount); // Выделяем список для хранения информации о дочерних элементах,
                                                     // размер которого соответствует количеству дочерних элементов (childsCount).
            for (int i = 0; i < childsCount; i++)
                childs.Add(ParseInf(rest, out rest)); // Для каждого дочернего элемента, если тип данных является структурой,
                                                      // мы вызываем рекурсивно функцию ParseInf, 
                                                      // передавая ей текущую позицию в массиве байтов rest. Функция ParseInf будет
                                                      // читать информацию о типе данных для каждого дочернего элемента и обновлять переменную rest, 
                                                      // чтобы она указывала на оставшуюся часть массива байтов после чтения информации
                                                      // о каждом дочернем элементе. Сохраненные объекты TypeInf для каждого
                                                      // дочернего элемента добавляются в список childs.
        }
