...
public class BinToCsvConverter : BaseDisposable
{
    readonly Stream _srcFile;
    readonly Stream _dstFile;
    readonly BinReader _reader;
    readonly CsvWriter _writer;

    public BinToCsvConverter(string srcBin, string dstCsv)
    {
        _srcFile = new FileStream(srcBin, FileMode.Open);
        _dstFile = new FileStream(dstCsv, FileMode.Create);
        _reader = new BinReader(_srcFile);
        _writer = new CsvWriter(_dstFile);
    }
